...
// ─── FUNCIÓN PRINCIPAL: RENDERIZAR CON FILTROS ───
...
function ordenarProductos(arr, orden) {
  ...
  switch (orden) {
    ...
    default:
      return copia; // relevancia = orden original
  }
}

// ─── FILTRAR POR CATEGORÍA (modificado para usar el estado) ───
...
function resetearFiltros() {
  ...
  const select = document.getElementById('filtroOrden');
  if (input) input.value = '';
  if (select) select.value = 'relevancia';
